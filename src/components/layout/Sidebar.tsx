    ...
    <aside className="sidebar">
      <div className="brand-mark">
        ...
        <div>
          ...
          <small style={{ color: 'var(--primary-strong)', fontWeight: 600 }}>Freshman Handbook</small>
        </div>
      </div>
      <div style={{ padding: '0 12px', marginBottom: '12px', fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em' }}>EXPLORE</div>
      <nav aria-label="หลัก" style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {navItems.map((item) => {
          const Icon = item.icon;
          return (
            <NavLink key={item.path} to={item.path} end={item.path === '/'}>
              <Icon size={20} style={{ opacity: 0.8 }} />
              <span style={{ paddingTop: '2px' }}>{item.label}</span>
            </NavLink>
          );
        })}
      </nav>
